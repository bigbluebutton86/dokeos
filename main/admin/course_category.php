...
if($action == 'add' || $action == 'edit')
{
	...
	echo '<a href="'.api_get_self().'?category='.Security::remove_XSS($category).'">'.Display::return_icon('folder_up.gif').get_lang("Back"); 
	if(!empty($category)){
	...
	if(!empty($errorMsg))
	{
		echo '<tr><td colspan="2">'.Display::display_normal_message($errorMsg).'</td></tr>';
	}
...
else
{
	...
	if(!empty($category) && empty($action))
	{
		...
		echo '<a href="'.api_get_self().'?category='.$parent_id['parent_id'].'">'.Display::return_icon('folder_up.gif').get_lang("Back");
