...
if (!empty($_GET['category']) && !in_array($_GET['category'], array('Plugins', 'stylesheets', 'Search')))
{
	...
	foreach($settings as $row) {
		...
		if ($row['scope'] != '-1' AND !is_null($row['scope'])) {
			...
			$form->addElement ( 'html', '<div style="float:right;padding-top:3px;margin-right:70px;" id="coursesettingdelegationdiv_' . $row ['variable'] . '" class="coursesettingdelegationdiv" title="<sub>'.get_lang('ClickToEnableOrDisable').'</sub>">
...
function handle_stylesheets()
{
	...
	echo '<div style="padding:0px;border:0px;"><iframe frameborder="0" src="style_preview.php" width="100%" height="350"  name="preview"></iframe></div>';
