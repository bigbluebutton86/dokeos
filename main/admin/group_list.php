...
function modify_filter($group_id,$url_params,$row)
{
	...
	if (api_is_platform_admin()) {
		$result .= '<a href="'.api_get_path(WEB_CODE_PATH).'admin/add_users_to_group.php?id='.$group_id.'">'.Display::return_icon('groupadd_22.png',get_lang('AddUsersToGroup')).'</a>';
		$result .= '<a href="group_edit.php?id='.$group_id.'">'.Display::return_icon('pixel.gif', get_lang('Edit'),array('class'=>'actionplaceholdericon actionedit')).'</a>&nbsp;&nbsp;';
...
else
{
				...
				case 'delete_group' :
					if (api_is_platform_admin()) {
						if (GroupPortalManager :: delete($_GET['group_id'])) {
                          echo get_lang('GroupDeleted');
						} else {
                          echo get_lang('CannotDeleteGroup');
						}
					}
					break;
				case 'lock' :
	...
	// Create a search-box
	$form = new FormValidator('search_simple','get','','',array('style'=> 'float:right'),false);
	...
	if (api_is_platform_admin()) {
		echo '<a href="'.api_get_path(WEB_CODE_PATH).'admin/group_add.php">'.Display::return_icon('groupadd_32.png',get_lang('AddUsers')).get_lang('AddGroups').'</a>';
	}
     $form->display();
