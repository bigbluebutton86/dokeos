...
if ($action == "updateRecordsListings"){
	...
	echo '<script>window.location.href = "viewforumcategory.php?'.api_get_cidReq().'&forumcategory='.$_GET['forumcategory'].'";</script>';
}
...
if (api_is_allowed_to_edit(false,true)) {
    if ($action_forums == 'add') echo '<div id="content">'; // Start content
	...
	$forum_categories_list='';
		...
		foreach ($forum_list as $key=>$forum) {
			...
			// note: this can be speeded up if we transform the $forum_list to an array that uses the forum_category as the key.
				...
				if (api_is_allowed_to_edit ()) echo "<td width='5%'><img src='../img/drag-and-drop.png'></td>";
				// Showing the image
