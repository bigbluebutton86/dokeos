	...
	if (is_array($addedresource))
	{
		...
		foreach ($addedresource as $resource)
		{
			...
			if ($showdeleteimg==1)
			{
				...
				//action=$action&id=$id&
				//echo "action=$action&amp;id=$id&amp;originalresource=no&amp;resourceaction=removeresource&amp;key=".key($addedresource)."'><img src='../img/delete.png' border='0' alt='resource ".get_lang('Delete')."' /></a><br />";
