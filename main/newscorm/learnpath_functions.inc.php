...
*	Due to the fact that the learning path has been built upon the resoucelinker,
...
*	Item (?) = Item    = Step
*
*	@author  Denes Nagy, main author
*	@author  Roan Embrechts, some code cleaning
*	@author	Yannick Warnier, multi-level learnpath behaviour + new SCORM tool
*	@access  public
		...
		foreach ($tree[$parent_item_id] as $row)
		{
			if ($row['item_type'] === 'dokeos_chapter')
			{
				...
				if (is_allowed_to_edit())
				{
					...
					echo "  <td align='center'>&nbsp;</td>\n";
					//echo "  <td align='center'>"."<a href='".api_get_self()."?lp_id=$learnpath_id&amp;action=editmodule&amp;id=".$row["id"]."'&SQMSESSID=36812c2dea7d8d6e708d5e6a2f09b0b9>"."<img src=\"../img/edit.png\" border=\"0\" title=\"$lang_edit_learnpath_module\">"."</a>"."</td>\n";
					echo "  <td align='center'>"."<a href='".api_get_self()."?lp_id=$learnpath_id&action=edititem&id=".$row["id"]."'>"."<img src=\"../img/edit.png\" border=\"0\" title=\"$lang_edit_learnpath_module\">"."</a>"."</td>\n";

					//echo "  <td align='center'>"."<a href='".api_get_self()."?lp_id=$learnpath_id&amp;action=deletemodule&amp;id=".$row["id"]."'&SQMSESSID=36812c2dea7d8d6e708d5e6a2f09b0b9 onclick=\"return confirmation('".$row['chapter_name']."');\">"."<img src=\"../img/delete.png\" border=\"0\" title=\"$lang_delete_learnpath_module\">"."</a>"."</td>\n";
					echo "  <td align='center'>"."<a href='".api_get_self()."?lp_id=$learnpath_id&action=delete_item&id=".$row["id"]."' onclick=\"return confirmation('".$row['title']."');\">"."<img src=\"../img/delete.png\" border=\"0\" title=\"$lang_delete_learnpath_module\">"."</a>"."</td>\n";
				}
			...
			else //if //($row['item_type'] === 'item')
			{
				...
				if (is_allowed_to_edit())
				{
					...
					// edit
					echo "<td align='center'>"."<a href='".api_get_self()."?lp_id=$learnpath_id&amp;action=edititem&amp;id=".$row_items["id"]."'&SQMSESSID=36812c2dea7d8d6e708d5e6a2f09b0b9>"."<img src=\"../img/edit.png\" border=\"0\" title=\"$lang_edit_learnpath_item\">"."</a>"."</td>";

					// delete
					echo "<td align='center'>"."<a href='".api_get_self()."?lp_id=$learnpath_id&action=deleteitem&id=".$row_items["id"]."'&SQMSESSID=36812c2dea7d8d6e708d5e6a2f09b0b9>"."<img src=\"../img/delete.png\" border=\"0\" title=\"$lang_delete_learnpath_item\" onclick=\"return confirmation('".$row_items["item_type"]."');\">"."</a>";
				}
				$i ++;
				echo "</td></tr>";
...
function display_all_learnpath()
{
	...
	while ($row = Database::fetch_array($result))
	{
		...
		if (is_allowed_to_edit())
		{
			...
			echo "<td bgcolor=\"$color2\" align=center><a href='".api_get_self()."?action=editpath&id=".$row["lp_id"]."'&SQMSESSID=36812c2dea7d8d6e708d5e6a2f09b0b9><img src=\"../img/edit.png\" border=\"0\" title=\"$lang_edit_learnpath\"></a></td>";
			echo "<td bgcolor=\"$color2\" align=center><a href='".api_get_self()."?action=deletepath&id=".$row["lp_id"]."'&SQMSESSID=36812c2dea7d8d6e708d5e6a2f09b0b9><img src=\"../img/delete.png\" border=\"0\" title=\"$lang_delete_learnpath\" onclick=\"return confirmation('".$row2['learnpath_name']."');\"></a></td>";
			$id = $row["lp_id"];
...
function display_learnpath_items($categoryid)
{
	...
	while ($row_items = Database::fetch_array($result_items))
	{
		...
		if (is_allowed_to_edit())
		{
			...
			echo "<a href='".api_get_self()."?lp_id=$learnpath_id&action=deleteitem&id=".$row_items["id"]."'&SQMSESSID=36812c2dea7d8d6e708d5e6a2f09b0b9><img src=\"../img/delete.png\" border=\"0\" title=\"$lang_delete_learnpath_item\" onclick=\"return confirmation('".$langThisItem."');\"></a>";
		}
		$i ++;
		echo "</td></tr>";
	}
