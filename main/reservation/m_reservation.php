...
// Tool name
$tool_name = get_lang('ViewBookingPeriods');
 ...
 */
function modify_filter($id,$url_params,$row) {
		$out = ' <a href="m_reservation.php?action=accept&amp;rid='.$id.'" alt="'.get_lang('ManageBooking').'">'.Display::return_icon('students.gif',get_lang('ManageBooking')).'</a>';
		$out .= '<a href="m_reservation.php?action=edit&amp;id='.$id.'" alt="'.get_lang('EditBookingPeriod').'">'.Display::return_icon('edit.png',get_lang('EditBookingPeriod')).'</a>';
		$out .= ' <a href="m_reservation.php?action=delete&amp;id='.$id.'" alt="'.get_lang('DeleteBookingPeriod').'" onclick="javascript:if(!confirm('."'".addslashes(api_htmlentities(get_lang("ConfirmDeleteBookingPeriod")))."'".')) return false;">'.Display::return_icon('delete.png',get_lang('DeleteBookingPeriod')).'</a>';
...
switch ($_GET['action']) {
	case 'overviewsubscriptions' :
		...
		// tool name
		//$tool_name = get_lang('OverviewSubscriptions');
	...
	case 'accept' :
		...
		// getting the id of the resource
		$resource_id = Rsys::get_reservation($_GET['rid'],'ASSOC', array('i.id'));
		
		// getting all the information of the resource
		$resource = Rsys::get_item($resource_id[0]['id']);
		
		// getting all the information about the resource category
		$resource_category = Rsys::get_category($resource['category_id']);
		
		// breadcrumbs
		$interbreadcrumb[] = array ('url' => 'm_category.php', 'name' => get_lang('BookingSystem'));
		$interbreadcrumb[] = array ('url' => 'm_item.php?cat='.$resource['category_id'], 'name' => $resource_category['name']);
		$interbreadcrumb[] = array ('url' => 'm_reservation.php?action=overviewsubscriptions&resource='.$resource['id'], 'name' => $resource['name']);
		$interbreadcrumb[] = array ('url' => 'm_reservation.php?action=overviewsubscriptions&resource='.$resource['id'], 'name' => get_lang('ViewBookingPeriods'));
		...
		// header
		Display :: display_header(get_lang('Bookers'));
	...
	case 'add' :
		...
		// tool name
		$tool_name = get_lang('AddNewBookingPeriod');
		...
		echo '<input type="hidden" name="action" value="add"/>';
			echo '<div class="row">
					<div class="form_header">'.get_lang('Step').' 1: '.get_lang('SelectResourceTypeAndResource').'</div>
				  </div>';
		echo '<div class="row">';
		...
		$form->addElement('header','',get_lang('Step').' 2: '.get_lang('AddNewBookingPeriod'));
		...
		if ($form->validate()) {
			...
			// we check if the item is alvays_available or not
			$item = Rsys :: get_item($values['itemid']);
			if ($item['always_available'] == 1) 
			{
				Display::display_error_message(get_lang('ItemAlwaysAvailableError'));
			}
			else 
			{			
				if ($values['forever'] == 0) 
			...
			switch($msg_number) {
				case 0 :
						Display :: display_confirmation_message(Rsys :: get_return_msg(get_lang('BookingPeriodAdded'), "m_reservation.php", $tool_name),false);
					break;
				case 1 :
				...
				case 6:
					...
					break;
				case 7:
						Display :: display_error_message(get_lang('BookingPeriodTimePickerError3'),false);
					break;
				default :
					break;
			}
	...
	case 'edit' :
		...
		else
		{
			...
			if ($form->validate()) {
				...
				switch ($msg_number) {
					case 0 :
						Display :: display_confirmation_message(Rsys :: get_return_msg(get_lang('BookingPeriodEdited'), "m_reservation.php", $tool_name),false);
						break;
					case 1 :
						Display :: display_error_message(str_replace('#END#', "<b>".$GLOBALS['end_date']."</b>",str_replace('#START#', "<b>".$GLOBALS['start_date']."</b>",get_lang('BookingPeriodDateOverlap'))),false);
						break;
					case 2 :
						Display :: display_normal_message(get_lang('BookingPeriodSubscribeUntilAfterStart'),false);
						break;
...
*/
$interbreadcrumb[] = array ('url' => 'm_category.php', 'name' => get_lang('BookingSystem'));
if (isset($_GET['resource']) AND is_numeric($_GET['resource']))
{
	// getting all the information of the resource
	$resource = Rsys::get_item(Security::Remove_XSS($_GET['resource']));
...
// displaying the forms and feedback messages (if any)
echo $buffer;
	
// actions
echo '<div class="actions">';
echo '<form id="cat_form" action="m_reservation.php" method="get" style="float: right;">';
echo '<input type="text" name="keyword" /><button type="submit" class="search" value="'.get_lang('Search').'" >'.get_lang('Search').'</button></form>';
...
echo '<a href="m_item.php?action=add&amp;cat='.Security::Remove_XSS($_GET['cat_id']).'">'.Display::return_icon('cube_add.png',get_lang('AddNewResource')).get_lang('AddNewResource').'</a>';
...
echo '<a href="m_reservation.php?action=add&amp;cat_id='.Security::Remove_XSS($_GET['cat_id']).'">'.Display::return_icon('calendar_add.gif',get_lang('AddNewBookingPeriod')).get_lang('AddNewBookingPeriod').'</a>';
echo '</div>';
