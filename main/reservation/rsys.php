...
// initialisation of the reservation system
Rsys :: init();
...
class Rsys {
	...
	*/
	 ...
	 *  @param  -   String     $courseCode  The course-Code (optional)
	 */
	function edit_item($id, $name, $description, $category, $course = "", $always_available) {
		if (!Rsys :: item_allow($id, 'edit'))
			return false;
		if (!Rsys :: check_item($name, $category, $id))
			return false;
		$sql = "UPDATE ".Rsys :: getTable("item")." SET category_id='".Database::escape_string($category)."',course_code='".Database::escape_string($course)."',name='".Database::escape_string($name)."',description='".Database::escape_string($description)."', always_available='".Database::escape_string($always_available)."' WHERE id ='".Database::escape_string($id)."'";
		Database::query($sql, __FILE__, __LINE__);
		return $id;
	}
	...
	function get_item($id = null, $orderby = "name ASC") {
		...
		if (!empty ($id)) {
			if (!Rsys :: item_allow($id, 'view') AND !api_is_platform_admin()) {
				return false;
			}
			$sql .= " WHERE i.id = '".$id."'";
		} else
	...
	*/
	...
	function get_table_reservations($from, $per_page, $column, $direction) {
		...
		// show only the booking periods of a certain resource
        if (is_numeric($_GET['resource']) AND !empty($_GET['resource']))
        {
        	$sql .= "AND i.id = '".Database::escape_string(Security::Remove_XSS($_GET['resource']))."'";
        }
        // show only the booking periods of a certain resource category
        if (is_numeric($_GET['resource_category']) AND !empty($_GET['resource_category']))
        {
        	$sql .= "AND i.category_id = '".Database::escape_string(Security::Remove_XSS($_GET['resource_category']))."'";
        }         	
	...
	function get_num_reservations() {
		...
		// count only the booking periods of a certain resource
        if (is_numeric($_GET['resource']) AND !empty($_GET['resource']))
        {
        	$sql .= "AND i.id = '".Database::escape_string(Security::Remove_XSS($_GET['resource']))."'";
        }
        // count only the booking periods of a certain resource category
        if (is_numeric($_GET['resource_category']) AND !empty($_GET['resource_category']))
        {
        	$sql .= "AND i.category_id = '".Database::escape_string(Security::Remove_XSS($_GET['resource_category']))."'";
        }        
	...
	function get_reservation($id,$return_type = 'NUM',$fields=array()) {
		$id = Database::escape_string($id);
		...
		$result = Database::query($sql, __FILE__, __LINE__);
		while ($array = Database::fetch_array($result, $return_type))
			$arr[] = $array;
		return $arr;
	}
	...
	*/
	 ...
	 */
	function add_subscription($reservation_id, $user_id, $accepted, $item_id) {
		$reservation_id = Database::escape_string($reservation_id);
	 ...
	 * @param -		int		$reservation_id		The id off the reservation
	 */
	function add_subscription_timepicker($reservation_id, $user_id, $start_date, $end_date, $accepted, $min, $max, $item=0) {
		...
		//echo $start_date.' tot '.$end_date.'<hr />';
		// check if we have a reservation period that exceeds the 
		if (date('d',Rsys :: mysql_datetime_to_timestamp($start_date)) <> date('d',Rsys :: mysql_datetime_to_timestamp($end_date)) OR date('m',$start_date) <> date('m',$end_date) OR date('Y',$start_date) <> date('Y',$end_date))
		{
			$count = 0;
			$temp_end_date_datetime = substr($start_date,0,10).' 23:59:59';
			while (Rsys :: mysql_datetime_to_timestamp($temp_end_date_datetime) < Rsys :: mysql_datetime_to_timestamp($end_date) OR $count>10)
			{
				//echo 'van: '.$start_date.' tot: '.$temp_end_date_datetime.'<br />';
				$sql = "INSERT INTO ".Rsys :: getTable("subscription")." (user_id,reservation_id,accepted,start_at,end_at,item_id) VALUES ('".Database::escape_string($user_id)."','".Database::escape_string($reservation_id)."','". ($accepted ? '1' : '0')."','".$start_date."','".$temp_end_date_datetime."','".Database::escape_string($item)."')";
				//echo $sql.'<br />';	
				api_sql_query($sql, __FILE__, __LINE__);					
				$temp_end_date_datetime = Rsys :: timestamp_to_datetime(Rsys :: mysql_datetime_to_timestamp($temp_end_date_datetime) + (24*60*60));
				$start_date = Rsys :: timestamp_to_datetime(Rsys :: mysql_datetime_to_timestamp(substr($start_date,0,10).' 00:00:00') + (24*60*60));
				$count++;
			}
			
			// and finally we add the last remaining bit of the day
			//echo 'van: '.$start_date.' tot: '.$end_date.'<br />';
			$sql = "INSERT INTO ".Rsys :: getTable("subscription")." (user_id,reservation_id,accepted,start_at,end_at,item_id) VALUES ('".Database::escape_string($user_id)."','".Database::escape_string($reservation_id)."','". ($accepted ? '1' : '0')."','".$start_date."','".$end_date."','".Database::escape_string($item)."')";
			//echo $sql.'<br />';	
			api_sql_query($sql, __FILE__, __LINE__);		
		}
		else 
	...
	function get_item_always_available_subscriptions($from, $till, $itemid) {
		$sql = "SELECT * FROM ".Rsys :: getTable('subscription')." WHERE item_id = '".Database::escape_string($itemid)."' AND (start_at='0000-00-00 00:00:00' OR (start_at<='".$from."' AND end_at>='".$from."') OR (start_at>='".$from."' AND start_at<='".$till."')) ORDER BY start_at ASC";
		...
		while ($array = mysql_fetch_array($result, MYSQL_ASSOC)) {
			...
			$rarr[(int)substr($array['start_at'],8,2)][] = $array;
		}
		return $rarr;
	}	
	...
	function menu()
	{
        ...
        {
        	case api_get_path(SYS_CODE_PATH).'reservation/mysubscriptions.php':
        		$subscription_active = ' class="active" ';
        		break;
        	case api_get_path(SYS_CODE_PATH).'reservation/reservation.php':
        		$reservation_active = ' class="active" ';
        		break;
        	case api_get_path(SYS_CODE_PATH).'reservation/m_item.php':
        		$item_active = ' class="active" ';
        		break;
        	case api_get_path(SYS_CODE_PATH).'reservation/m_reservation.php':
        		$period_active = ' class="active" ';
        		break;
        	case api_get_path(SYS_CODE_PATH).'reservation/subscribe.php':
        		$reservation_active = ' class="active" ';
        		break;        		
        }
        
		echo '<div id="tabbed_menu">';
		echo '	<ul id="tabbed_menu_tabs">';
		echo '		<li><a href="mysubscriptions.php"'.$subscription_active.'>'.get_lang('MyRes').'</a></li>';
		echo '		<li><a href="reservation.php"'.$reservation_active.'>'.get_lang('ResMan').'</a></li>';
		require_once('rsys.php');
		if(api_is_platform_admin() || Rsys :: check_user_status() == 1)
		{ // Only for admins & teachers...
			echo '		<li><a href="m_item.php"'.$item_active.'>'.get_lang('ItemsMan').'</a></li>';
			echo '		<li><a href="m_reservation.php"'.$period_active.'>'.get_lang('ResPeriodMan').'</a></li>';	
		}
		echo '	</ul></div>';		
	}
