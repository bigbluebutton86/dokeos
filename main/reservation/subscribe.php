...
if (is_numeric($_GET['item']) AND !empty($_GET['item']))
{
	$item = Rsys :: get_item($_GET['item']);
	// mimicing the reservation
	$reservation[0][5]=$_GET['timestart'];
	$reservation[0][6]=(int)$_GET['timestart']+(24*60*60);
	$reservation[0][7]='0000-00-00 00:00:00';
	$reservation[0][8]='0000-00-00 00:00:00';
	$reservation[0][8]=$item['name'];
	$reservation[0][12]=60;
}
if ($reservation[0][9] < $reservation[0][4] OR $item['always_available'] == '1'){
	ob_start();
	$form = new FormValidator('reservation', 'post', 'subscribe.php?rid='.Security::remove_XSS($_GET['rid']).'&item='.Security::remove_XSS($_GET['item']));
	$form->addElement('hidden', 'timepicker', $reservation[0][11]);
	$form->addElement('hidden', 'accepted', $reservation[0][3]);
	if ($reservation[0][11] == 1 OR $item['always_available'] == '1') {
		...
		if (Database::num_rows($result) != 0 AND $item['always_available'] <> '1'){
			$start_end = "<ul>";
	...
	if ($form->validate()) {
		...
		if ($values['timepicker'] == 0 AND $item['always_available'] <> '1') 
		{
			...
			$result = Rsys :: add_subscription_timepicker($_GET['rid'], api_get_user_id(), $values['startpicker'], $values['endpicker'],$values['accepted'],$values['min'],$values['max'],$_GET['item']);
			switch ($result) {
				case 0 :
