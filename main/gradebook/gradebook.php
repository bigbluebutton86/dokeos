...
// Allow display each gradebook
$htmlHeadXtra[] = '<script>
  $(document).ready(function (){
    $("#selectcat").change(function(){
      val = $(this).val();
      // Split the select val
      vals = val.split("|__|");
      // Get the category ID
      cat_id = vals[0];
      if(cat_id > 0){
        $("#selectcat_id").val(cat_id);
        // Set the course ID
        $("#cidreq_id").val(vals[1]);
        $("#gradebook_form").submit();
      }else {
        document.location = "?selectcat=0";
      }
    });
...
if (isset($_GET['view']) && $_GET['view'] == 'presence') {
	$addparams['view'] = 'presence';	
}
