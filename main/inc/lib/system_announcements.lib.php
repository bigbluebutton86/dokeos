...
class SystemAnnouncementManager
{
	 ...
	 * @param int $id The identifier of the announcement to display
	 */
	function count_announcements($visible, $id = -1)
	{
		$user_selected_language = api_get_interface_language();
		$db_table = Database :: get_main_table(TABLE_MAIN_SYSTEM_ANNOUNCEMENTS);
		$sql = "SELECT *, DATE_FORMAT(date_start,'%d-%m-%Y') AS display_date FROM ".$db_table." WHERE (lang='$user_selected_language' OR lang IS NULL) AND ((NOW() BETWEEN date_start AND date_end) OR date_end='0000-00-00') ";
		...
		$sql .= " ORDER BY date_start DESC LIMIT 0,7";
		$announcements = Database::query($sql,__FILE__,__LINE__);
		$numberofrows = Database::num_rows($announcements);
		return $numberofrows;
	}
	...
	function send_system_announcement_by_email($title,$content,$teacher, $student)
	{
		...
		while($row = Database::fetch_array($result,'ASSOC'))
		{
			...
			$content = str_replace('\n','',$content);
			api_mail_html(api_get_person_name($row['firstname'], $row['lastname'], null, PERSON_NAME_EMAIL_ADDRESS), $row['email'], api_html_entity_decode($title, ENT_QUOTES, $charset), api_html_entity_decode($content, ENT_QUOTES, $charset), api_get_person_name($_user['firstName'], $_user['lastName'], null, PERSON_NAME_EMAIL_ADDRESS), api_get_setting('emailAdministrator'), api_get_setting('emailAdministrator'));
		}
	}
}
?>
