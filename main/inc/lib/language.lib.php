...
class LanguageManager {
     ...
     * Get countries
     */
    public static function get_countries($code = null) {        
        $countries = array();
        ...
        $rs = Database::query("SELECT numcode, langvar FROM $tbl_country $condition");
        if (Database::num_rows($rs) > 0) {            
            while ($row = Database::fetch_object($rs)) {
                $countries[$row->numcode] = get_lang($row->langvar);
            }
        }        
        return $countries;
    }
