...
// check if this is a bar chart and what kind (i.e., vertical /horizontal)
// if a bar chart, redefine the distance variable to such that mousing over the entire bar will return the neighbor object
if (r.constructor == $.jqplot.BarRenderer) {
    if (s.barDirection == 'vertical') {
        if (y >= (0+threshold) && y <= (plot.grid._height-threshold)) {
            d = Math.abs( x-p[0] );
        }
    } else {
        if (x >= (0+threshold) && x <= (plot.grid._width-threshold)) {
            d = Math.abs( y-p[1] );
        }
    }
}
