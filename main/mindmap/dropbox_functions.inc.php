...
function store_addcategory()
{
	...
	if (!$_POST['edit_id'])
	{
		...
		if (Database::num_rows($result)==0)
		{
			...
			$base_work_dir = api_get_path(SYS_COURSE_PATH).$course_name[1].'/document';
			$dir_name = '/mindmaps/'.Database::escape_string(Security::remove_XSS($_POST['category_name']));
			$post_dir_name = Database::escape_string(Security::remove_XSS($_POST['category_name']));
			...
			if(!$created_dir)
			{
				return array('type' => 'error', 'message'=>get_lang('CannotCreateDir'));
			}
...
function display_add_form()
{
	...
	<div class="row">
		...
		<div class="formw">
				...
				<input type="hidden" name="curdirpath" value="<?php echo $_GET['curdirpath']; ?>" />
				<?php
				if ($origin=='learnpath')
				{
...
//	}
	...
	*/
	...
	//  echo '<option value="user_'.$_user['user_id'].'">'.dropbox_lang("justUploadInSelect").'</option>';
		...
		echo '</select>
		<img style="vertical-align:top;" src="../img/mindmap_128.png">
		</div>
	</div>';
...
function store_add_dropbox()
{
    ...
    */
    ...
    {	
		$_POST["recipients"] = 'user_'.$_user['user_id'];     
    }
  ...
  /*  if(!filter_extension($dropbox_filename))
	...
	$dropbox_filename_ext = pathinfo($dropbox_filename, PATHINFO_EXTENSION);	
	$allow_extension_files=array('png', 'jpg', 'gif', 'jpeg','xmind');
	if(!in_array($dropbox_filename_ext,$allow_extension_files))
	{
		return get_lang('UplUnableToSaveFileFilteredExtension');
	}
	...
	$course_name = explode("=",api_get_cidReq());
	$mindmap_syspath = api_get_path(SYS_COURSE_PATH).$course_name[1].'/document/mindmaps';
	$thumbnail_width = 250;
	...
	/*	copy($mindmap_path . '/' . $dropbox_filename, $mindmap_syspath . '/thumbs/' . $dropbox_filename);		
		...
		create_thumbnail($image,$thumbnail_width,$thumbnail_height);		*/
	}
	elseif($dropbox_filename_ext == 'xmind')
	{
		require_once '../inc/lib/mindmapConverter.php';
		@move_uploaded_file( $dropbox_filetmpname, $mindmap_syspath . '/xmind/' . $dropbox_filename);
		$converter = new MindmapConverter('127.0.0.1',1984);
		$source = $mindmap_syspath . '/xmind/' . $dropbox_filename;
		$tmp_filename = pathinfo($dropbox_filename);
		$destination = $mindmap_path . '/' . $tmp_filename['filename'].'.png';
		$converter->convertToPng($source, $destination);
	/*	copy($mindmap_syspath . '/' . $dropbox_filename, $mindmap_syspath . '/thumbs/' . $dropbox_filename);		
		$image = $mindmap_syspath . '/thumbs/' . $dropbox_filename;
		create_thumbnail($image,$thumbnail_width,$thumbnail_height);*/
	}
	...
	$_SESSION['curdirpath'] = $curdirpath;
...
/*	if(empty($curdirpath))
	{
	$desired_filename = '/mindmaps/'.$dropbox_filename;
	}
	else
	{
	$desired_filename = '/mindmaps/'.$curdirpath.'/'.$dropbox_filename;
	}	
	$document_id = add_document($_course, $desired_filename,'file',$dropbox_filesize,$dropbox_filename);
	if ($document_id)
	{
	...
	api_item_property_update($_course,TOOL_DOCUMENT,$document_id,'MindmapCreated',$_user['user_id'],$to_group_id,$to_user_id,null,null,$current_session_id);		
	}*/
...
function feedback_form()
{
	...
	echo $sql="SELECT * FROM ".$dropbox_cnf["tbl_person"]." WHERE file_id='".Database::escape_string($_GET['id'])."'";
...
function create_thumbnail($image,$thumbnail_width,$thumbnail_height)
{
	   ...
	   imagecopyresized( $tmp_img, $img, 0, 0, 0, 0, $thumbnail_width, $thumbnail_height, $width, $height );

	   // save thumbnail into a file
	   imagepng( $tmp_img, $image );
}

/**
 * Checkf if user is allowed to download the file
 */

function check_if_user_is_allowed_to_download_file ($file_id, $user_id) {
  $propTable = Database::get_course_table(TABLE_ITEM_PROPERTY);
  $allowed_to_download=false;
  $sql = "SELECT * FROM ".$propTable." WHERE ref='".Database::escape_string($file_id)."' AND (insert_user_id = '".Database::escape_string($user_id)."' OR to_user_id = '".Database::escape_string($user_id)."')";
  $result=Database::query($sql);
  if (Database::num_rows($result)>0) {
      $allowed_to_download=true;
  }
  return $allowed_to_download;
}