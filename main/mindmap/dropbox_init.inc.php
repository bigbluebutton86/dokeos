...
{
$javascript .= "<script src='".api_get_path(WEB_LIBRARY_PATH)."javascript/floating-gallery.js' type='text/javascript' language='javascript'>";
}
...
if (($_POST['action']=='download_received' || $_POST['action']=='download_sent') and !$_POST['store_feedback'])
{
	...
	{
		header ('location: index.php?view='.Security::remove_XSS($_GET['view']).'&error=CheckAtLeastOneFile');
		exit;
	}
	else
...
{
	$interbreadcrumb[] = array ("url" => "../mindmap/index.php", "name" => dropbox_lang("dropbox", "noDLTT"));
	$nameTools = get_lang('ReceivedFiles');

	if ($_GET['action'] == 'addreceivedcategory')
	{
		$interbreadcrumb[] = array ("url" => "../mindmap/index.php?view=received", "name" => get_lang("ReceivedFiles"));
...
if ($_GET['view']=='sent' OR empty($_GET['view']))
{
	...
	{
		$interbreadcrumb[] = array ("url" => "../mindmap/index.php?view=sent", "name" => get_lang("SentFiles"));
		$nameTools = get_lang('AddNewCategory');
	}
	if ($_GET['action'] == 'add')
	{
...
//script to implement Drag and Drop in Mindmap
echo '<script type="text/javascript">		
	...
	saveImageOrder(orderString);
}

function saveImageOrder(str)
	{
		var orderString = "";
		var objects = document.getElementsByTagName(\'div\');
		...
		if(str != orderString)
		{
			window.location.href="index.php?'.api_get_cidReq().'&view='.$_GET['view'].'&dispaction=updateRecordsListings&order="+orderString;
		}
	}
</script>';	
?>