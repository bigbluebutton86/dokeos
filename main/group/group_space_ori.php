...
function display_group_assignments($current_group) {
	if ( $current_group['work_state'] != TOOL_NOT_AVAILABLE) {
		echo '<a href="../work/work.php?'.api_get_cidreq().'&amp;toolgroup='.$current_group['id'].'">'.Display::return_icon('works.gif', get_lang('GroupWork'),array('align'=>'middle')).'&nbsp;'.get_lang('GroupWork').'</a>&nbsp;&nbsp;';
	}
}
...
if (api_is_allowed_to_edit(false,true) OR GroupManager :: is_user_in_group($_SESSION['_user']['user_id'], $current_group['id'])) {
	...
	if ( $current_group['work_state'] == TOOL_PUBLIC )
	{
		...
		$tools .= "<a href=\"../work/work.php?".api_get_cidreq()."&amp;toolgroup=".$current_group['id']."\">".Display::return_icon('works.gif', get_lang("GroupWork"),array('align'=>'middle'))."&nbsp;".get_lang("GroupWork")."</a>&nbsp;&nbsp;";
	}
